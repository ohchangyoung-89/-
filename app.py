...
# [핵심] 큰 숫자를 한국어 단위(억, 만)로 가독성 있게 변환하는 함수
...
# [개선 1] 상태 카드를 최상단에 배치하여 즉각적인 부족 유무 파악
...
# [개선 2] 영문 단위(B, M) 제거 및 한국어 가독성 기반 Metric 배치
c1, c2, c3, c4 = st.columns(4)
c1.metric("월초 보통예금", format_won_korean(current_cash))
c2.metric("당월 만기유입", format_won_korean(income_maturity))
c3.metric("당월 예상지출", format_won_korean(final_expense), delta=f"-{final_expense:,.0f}", delta_color="inverse")
# 결과 값은 크게 강조
with c4:
...
# [개선 3] 가독성이 떨어지는 Gauge 차트를 대신하여, 자금 흐름을 보여주는 Bar 차트 도입
...
# 바 차트 생성 (Plotly Express 활용)
fig = px.bar(chart_data, x='항목', y='금액', color='유형', 
             text=chart_data['금액'].apply(format_won_korean), # 한국어 단위를 바 차트에 직접 표시
             color_discrete_map={'가용자금': '#1f77b4', '유출자금': '#FF4B4B', '최종결과': '#FFAA00' if expected_end_balance < 100000000 else '#2ca02c'},
             height=500)

# 차트 세부 설정
fig.update_traces(textposition='outside') # 텍스트를 바 바깥쪽에 표시
fig.update_layout(
    yaxis_title="금액 (원)",
    xaxis_title="",
    showlegend=False,
